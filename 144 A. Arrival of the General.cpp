...
// Submission   :: https://codeforces.com/contest/144/submission/189519291
...
void solve(){
	...
	auto min_ptr=min_element(all(v), [](int mn, int b){return mn<=b;});
	auto max_ptr=max_element(all(v), [](int mx, int b){return mx<b;});

	if(max_ptr<min_ptr)
		cout<<(max_ptr-v.begin())+(v.end()-min_ptr)-1;
	else
		cout<<(max_ptr-v.begin())+(v.end()-min_ptr)-2;
}
...
void solve(){
	int n; cin>>n;
	int mx_i=-1, mn_i=-1, mx=-1, mn=MX;

	for(int i=1, val; cin>>val; i++){
		if(val>mx){
			mx_i=i; mx=val;
		}
		if(val<=mn){
