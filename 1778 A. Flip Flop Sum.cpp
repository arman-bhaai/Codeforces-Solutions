...
// Contest ID   :: 1778
// Problem Name :: A. Flip Flop Sum
// Problem URL  :: https://codeforces.com/contest/1778/problem/A
// Submission   :: 
...
void solve(){
	...
	int sm=0;
	forn(i,n) sm+=v[i];
	bool single=false, double_=false;
	forab(i,1,n){
		if(v[i]==-1 || v[i-1]==-1) single=true;
		if(v[i]==-1 && v[i-1]==-1) double_=true;
	}
	if(double_) cout<<sm+4;
	else if(single) cout<<sm;
	else cout<<sm-4;
}
