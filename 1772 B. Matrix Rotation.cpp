...
// Submission   :: https://codeforces.com/contest/1772/submission/192146253
...
void solve(){
	...
	cout<<((a<b&&a<c&&d>b&&d>c)||(b<a&&b<d&&c>a&&c>d)||
		(d<b&&d<c&&a>b&&a>b&&a>c)||(c<a&&c<d&&b>a&&b>d)?"YES":"NO");
}
...
void solve(){
	vi v(4); getv(v);
	int mn=MX, mx=-MX, mn_i=-1, mx_i=-1;
	forn(i,4){
		if(v[i]>mx) {mx_i=i; mx=max(mx,v[i]);}
		if(v[i]<mn) {mn_i=i; mn=min(mn,v[i]);}
	}
	cout<<((mn_i==0&&mx_i==3)||(mn_i==1&&mx_i==2)||(mn_i==3&&mx_i==0)||(mn_i==2&&mx_i==1)?"YES":"NO");
}
...
void solve(){
	...
	bool ok=false;
	forn(i,4){
		if(v[0]<v[1]&&v[2]<v[3] && v[0]<v[2]&&v[1]<v[3]) ok=true;
		v2={v[2],v[0],v[3],v[1]};
		v=v2;
	}
	cout<<(ok?"YES":"NO");
...
void solve(){
	vi v(4); getv(v);
	swap(v[2],v[3]);
	int mn_i=min_element(all(v))-v.begin();
	int mx_i=max_element(all(v))-v.begin();
	cout<<(abs(mn_i-mx_i)==2?"YES":"NO");
}
*/