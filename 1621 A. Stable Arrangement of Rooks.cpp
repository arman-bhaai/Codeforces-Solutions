...
// Contest ID   :: 1621
// Problem Name :: A. Stable Arrangement of Rooks
...
void solve(){
	...
	if(n<(k*2-1)) {cout<<-1; return;}
	forn(i,n){
		if(i) cout<<en;
		forn(j,n){
			if(i==j && !(i&1) && k>0) {cout<<'R'; k--;}
			else cout<<'.';
		}
	}
}
...
void solve(){
	...
	vector<str> v(n, str(n,'.'));
	if(k>(n+1)/2) cout<<-1;
	else {
		forn(i,k) v[i*2][i*2]='R';
		forn(i,n) cout<<v[i]<<en;
	}
}
