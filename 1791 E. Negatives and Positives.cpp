...
// Problem URL  :: https://codeforces.com/contest/1791/problem/E
// Submission   :: https://codeforces.com/contest/1791/submission/192596324
...
void solve(){
	int n; cin>>n;
	vi v(n);
	ll mn=INF, cnt=0; ll sm=0;

	forn(i,n){
		cin>>v[i];
		if(v[i]<0) cnt++;
		sm+=abs(v[i]);
		mn=min(mn,(ll)abs(v[i]));
	}
	if(cnt&1) sm-=mn*2;
	cout<<sm;
	dbg(0^1, 1^3)
